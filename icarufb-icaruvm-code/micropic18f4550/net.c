...
/** Reserved buffer for configuration proccess. */
icr_uint8 net_buffer_out2[LP_BUF_SIZE];
...
void net_snd_force(icr_uint8 *data, icr_uint8 t){
	icr_uint8 i;
	if(net_buffer_out2[0] != 0){
		printf("(net_snd) E2\r\n");
		return;
	}
	if(t <= LP_BUF_SIZE){
		for(i=0;i<t;i++){
			net_buffer_out2[i] = data[i];
		}
		return;
	}else{
		printf("(net_snd) E1\r\n");
...
void net_run(){
	...
	if(net_oldt != time_sec_div_100){
		net_oldt = time_sec_div_100;
		if(net_i > 5){
			net_i = 0;
			if(net_tk == LP_ADDRESS){
				net_tk++;
				//send my buffer;
				if(net_buffer_out2[0] != 0){
					lp_send(net_buffer_out2, LP_NOTOKEN);
					net_buffer_out2[0] = 0;
				}else
				if(net_buffer_out[0] != 0){
	...
	if(lp_recv()){
		icaru_handle_netdata(lp_buffer);
		/*
